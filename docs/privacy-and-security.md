...
Selected files are read with browser file APIs, parsed in local WebAssembly, and rendered locally. genbank_viewer has no backend, upload, remote-fetch, or analytics code. A hosting provider, modified deployment, browser extension, or compromised dependency can change that property, so verify the deployed build before loading sensitive material.
